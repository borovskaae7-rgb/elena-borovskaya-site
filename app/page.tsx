...
export default function Home() {
  ...
  const onFiles = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles(images);
    if (selected.length > images.length) setError('Можно загрузить до 10 файлов только в формате PNG или JPG.');
    else setError('');
  };
  ...
  const submitAudit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setResult(null);
    if (!files.length) {
      setError('Загрузите хотя бы один скриншот сообщества.');
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? 'Не удалось провести аудит.');
      setResult(data.audit);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Не удалось провести аудит. Попробуйте ещё раз.');
    } finally {
      setLoading(false);
    }
  };
